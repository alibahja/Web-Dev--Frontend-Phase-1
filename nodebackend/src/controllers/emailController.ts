...
export const sendContactMessage = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        // Get user info
        const [users]: any = await req.app.get('pool').query(
        ...
        const result = await sendEmail(
            ...
            subject || `Contact from ${user.full_name}`,
            html
        );
...
export const sendTestEmail = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const result = await sendEmail(users[0].email, 'BiblioTech - Test Email', html);
...
export const sendAutoReply = async (userEmail: string, userName: string) => {
    ...
    return await sendEmail(userEmail, 'Thank you for contacting BiblioTech', html);
};