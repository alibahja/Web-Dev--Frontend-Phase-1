...
exports.getUserProfile = exports.updateProfilePicture = exports.deleteAccount = exports.getCurrentUser = exports.login = exports.register = void 0;
...
const register = async (req, res) => {
    try {
        ...
        // Insert user
        const [result] = await database_1.default.query('INSERT INTO users (full_name, email, password, role) VALUES (?, ?, ?, ?)', [full_name, email, hashedPassword, role]);
        res.status(201).json({
            success: true,
...
const login = async (req, res) => {
    try {
        ...
        // Get user
        const [users] = await database_1.default.query('SELECT id, full_name, email, password, role FROM users WHERE email = ?', [email]);
        if (users.length === 0) {
            res.status(401).json({
                success: false,
...
const getCurrentUser = async (req, res) => {
    try {
        const [users] = await database_1.default.query('SELECT id, full_name, email, role, created_at FROM users WHERE id = ?', [req.user?.userId]);
        if (users.length === 0) {
            res.status(404).json({
                success: false,
...
const deleteAccount = async (req, res) => {
    try {
        ...
        try {
            ...
            await connection.query('UPDATE communities SET admin_id = 1 WHERE admin_id = ?', [userId]);
...
const updateProfilePicture = async (req, res) => {
    try {
        ...
        await database_1.default.query('UPDATE users SET profile_picture = ? WHERE id = ?', [profilePictureUrl, userId]);
        res.json({
            success: true,
