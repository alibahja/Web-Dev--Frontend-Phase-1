...
import { extractList, normalizeGame } from "../api/normalize";

const GameCard = ({ game, darkMode }) => {
  const navigate = useNavigate();
  const g = normalizeGame(game);
  const img = g.image || "https://images.unsplash.com/photo-1587620962725-abab7fe55159?auto=format&fit=crop&q=80&w=800";
  return (
    <div
      role="button"
      tabIndex={0}
      ...
      onKeyDown={(e) => e.key === "Enter" && navigate(`/games/${g.id}`)}
...
const AllGames = ({ darkMode }) => {
  ...
  useEffect(() => {
    ...
    api
      .get("/api/games")
      .then((res) => {
        if (!cancel) setGames(extractList(res.data));
      })
      .catch(() => {
        if (!cancel) setGames([]);
      })
  ...
  const visibleGames = games.filter((game) =>
    (normalizeGame(game).title || "").toLowerCase().includes(searchQuery.toLowerCase())
  );
    ...
    <div className={`min-h-screen w-full transition-colors duration-500 overflow-x-hidden ${
      ...
      <div className="max-w-7xl mx-auto px-8 pb-32">
        <div className="flex items-center justify-between mb-10">
            ...
            <div className="text-sm font-mono opacity-50">{loading ? "…" : `${visibleGames.length} Results`}</div>
        </div>

        {loading ? (
          <p className="text-center opacity-60 py-12">Loading games…</p>
