...
const Footer = ({ darkMode }) => {
  return (
    <div className={`w-full py-6 border-t mt-10 ${
      darkMode 
...
const Home = ({darkMode}) => {
  ...
  useEffect(() => {
    ...
    const communityObserver = new IntersectionObserver(([entry]) => {
...
<nav className={`fixed top-0 w-full z-50 transition-colors duration-300 border-b ${darkMode ? 'bg-[#1E2740] border-[#2D3748]' : 'bg-white border-[#E2E8F0]'}`}>
  ...
  {['View your Profile', 'Join a Group', 'Settings'].map((item, index) => {
    // Determine the route for each item
    let route = '#';
    ...
    return (
      ...
      <section ref={communityRef} className="py-12 w-full px-6 md:px-10 overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-7xl mx-auto">
          ...
          {/* Box 1: Info Box (Animated Slide In From Left) */}
          <div className={`p-10 rounded-[2.5rem] transition-all duration-1000 transform 
            ...
            <p className={`text-lg leading-relaxed mb-8 opacity-80 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              ...
              <strong> Create your own community</strong>, host <strong>Discussion Boards</strong> for your favorite sagas, 
              and connect with readers who share your passion.
            </p>
            <div className="flex flex-wrap gap-4">
              <span className={`px-4 py-2 rounded-full text-sm font-bold ${darkMode ? 'bg-[#0A0F1F]' : 'bg-gray-100'}`}>#DiscussionBoard</span>
              <span className={`px-4 py-2 rounded-full text-sm font-bold ${darkMode ? 'bg-[#0A0F1F]' : 'bg-gray-100'}`}>#CreateCommunity</span>
            </div>
          </div>

          {/* Box 2: Visual Link Box (Animated Slide In From Right) */}
          <div className={`relative h-full min-h-[300px] rounded-[2.5rem] overflow-hidden transition-all duration-1000 delay-300 transform 
            ${showCommunity ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
            <div 
              className="absolute inset-0 bg-cover bg-center transition-transform duration-700 hover:scale-110" 
              style={{ backgroundImage: `url(${hero})` }} // Using hero as placeholder or your default photo
            >
